...
const statsJsonUriLocal =
  "https://raw.githubusercontent.com/Xiot/xiot.github.io/master/2024.json";
const statsJsonUri = "https://portal.xiot.ca/aoc-2024.json";
...
function calculateLocalScore(members) {
  const positionScore = (star) =>
    star == null ? 0 : star.gaveUp ? 0 : members.length - (star?.position ?? 0);
...
function buildMemberDayStats(member, day) {
  ...
  return {
    ...
    startTime,
    hasTimeOverride:
      member.completion_day_level[String(day)]?.start_ts !== undefined,
...
function buildDifferenceChart(el, members) {
  ...
  const allPoints = members
    ...
    .map((x) => getPoints(x, { allowEmpty: true }));
...
function buildRollingAverageChart(el, members) {
  ...
  const allPoints = members
    ...
    .map((points) =>
      range(25).map((day) =>
        points[day] == null ? undefined : avgLast(points, day, AVG_SIZE)
      )
    );
...
const getDayPoints = (member, opts) =>
  member.days.reduce((acc, day) => {
    const { allowEmpty = false } = opts ?? {};
    if (acc === undefined) {
      return [day.score];
...
function initialize(data) {
  ...
  document.getElementById("medals").appendChild(buildMedalGrid(members));
  ...
  days.forEach((day) => {
    ...
    append(grid, [
      div(
        { class: "day link value", onclick: () => showStatsForDay(day) },
        (day.day + 1).toString()
      ),
      ...
      div(
        { class: "time value" },
        formatDuration(starScoreTimeFn(winner.star2))
      ), //formatStarTime(winner.star2)),
...
function buildMedalGrid(members) {
  ...
  for (let member of members) {
    const row = range(25).map((i) => {
      ...
      const borderColor =
        pos2 !== -1 || pos1 <= 2 ? "transparent" : "rgba(0,0,0,0.2)";
...
function medalsForDay(day, position) {
  return (
    (isPosition(day.star1, position) ? 1 : 0) +
    (isPosition(day.star2, position) ? 1 : 0)
  );
}
...
function trophy(position, props) {
  ...
  if (position < 0 || position > 2)
    return div({ class: "trophy position" }, text(position + 1));
