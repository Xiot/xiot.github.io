...
const statsJsonUri = "https://portal.xiot.ca/aoc/2023/stats.json";
const overrideUri = (year) =>
  `https://portal.xiot.ca/aoc/${year}/overrides.json`;
...
function initialize(members) {
  ...
  const daySelector = createDaySelect(
    YEAR,
    selectedDay,
    user?.daysComplete ?? [],
    (e) => {
  ...
  const container = div({}, [
    ...
    node(
      ...
      members
        ...
        .map((m) => node("option", { value: m.id }, text(m.name)))
    ),
    node(
      "button",
      {
        onclick: () => {
          ...
          const node = Array.from(el.children).find(
            (x) => x.value === el.value
          );
...
function createLauncher(year, day) {
  ...
  const unlockTime = DateTime.fromObject(
    { year, month: 12, day },
    { zone: ZONE_NAME }
  );
  ...
  if (currentTime.toMillis() > unlockTime.toMillis()) {
    ...
    const countDown = node(
      "span",
      {},
      text(`Day ${day} unlocks ` + unlockTime.toRelative())
    );
    setInterval(() => {
...
function lastDayAttempted(member) {
  const lastDayIndex = range(25)
    .reverse()
    .findIndex((x) => member.completion_day_level[String(x + 1)]);
  return {
    id: member.id,
    name: member.name,
    lastDay: lastDayIndex === -1 ? 0 : 25 - lastDayIndex,
    daysComplete: Object.keys(member.completion_day_level).map((x) =>
      parseInt(x)
    ),
  };
}
